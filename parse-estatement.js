...
// Selamatkan objek transaksi dari JSON yang TERPOTONG (mis. keluaran model kena
// batas token). Objek transaksi berada di dalam array (baik dibungkus objek
// {"transactions":[...]} maupun array telanjang [...]). Strategi: mulai dari '['
// PERTAMA, lalu pungut setiap objek "{...}" berpasangan (kedalaman brace 0 relatif
// terhadap awal array); JSON.parse tiap kandidat, abaikan ekor yang tak utuh.
function salvageObjects(content) {
  const out = [];
  const s = String(content || '');
  const arrStart = s.indexOf('[');
  const scan = arrStart >= 0 ? s.slice(arrStart + 1) : s;
  ...
  for (let i = 0; i < scan.length; i++) {
    ...
    if (c === '{') { if (depth === 0) start = i; depth++; }
      ...
      depth--;
      if (depth === 0 && start >= 0) {
        const chunk = scan.slice(start, i + 1);
        try { out.push(JSON.parse(chunk)); } catch { /* lewati objek rusak */ }
        start = -1;
      }
    }
  }
  ...
  try {
    const upstream = await fetch(baseUrl + '/chat/completions', {
      ...
      body: JSON.stringify({
        model,
        temperature: 0,
        // PENTING: batas token keluaran. Tanpa ini, gpt-4o-mini memakai default
        // yang kecil (~4k) sehingga daftar transaksi PANJANG TERPOTONG di tengah
        // JSON → hanya SEBAGIAN transaksi yang kembali (bug "transaksi tidak lengkap").
        // 16384 = maksimum keluaran gpt-4o-mini, cukup untuk ratusan transaksi.
        max_tokens: 16384,
    ...
    // Deteksi pemotongan: bila model berhenti karena batas panjang, JSON bisa tak
    // utuh. Kita tetap coba selamatkan sebanyak mungkin objek transaksi dari teks.
    ...
    try {
      ...
      // JSON tak utuh (kemungkinan terpotong): pungut setiap objek transaksi lengkap
      // "{ ... }" yang masih valid, buang ekor yang terpotong.
      arr = salvageObjects(content);
    }
    res.setHeader('Cache-Control', 'no-store');
    res.status(200).json({ transactions: Array.isArray(arr) ? arr : [], truncated });
  } catch (e) {
